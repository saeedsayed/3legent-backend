...
const getMediaLibrary = async (req, res, next) => {
    const mediaLibrary = await MediaLibrary.find({ folderTitle: { $ne: "profile_pictures" } });
    res.status(200).json({
        status: "success",
        data: {
            mediaLibrary
...
const getFolders = async (req, res, next) => {
    const folders = await MediaLibrary.find({ folderTitle: { $ne: "profile_pictures" } }).select('-files');
    res.status(200).json({
        status: "success",
        data: {
            folders
...
const uploadFiles = async (req, res, next) => {
    ...
    if (folder.folderTitle === "profile_pictures") {
        const err = appError.create("You cannot upload files to this folder", 400, STATUS.FAIL)
        return next(err)
    }
    // upload files to cloudinary
    const arrOfFilesPaths = Promise.all(arrOfMedia.map(async (file) => {
        const filePath = await cloudinary.uploader.upload(file.tempFilePath, { folder: folder.folderTitle })
...
const createFolder = async (req, res, next) => {
    ...
    const existingFolder = await MediaLibrary.findOne({ folderTitle: lowerFolderTitle })
    if (existingFolder) {
        const err = appError.create("Folder already exists", 400, STATUS.FAIL)
        return next(err)
    }
    const newFolder = await MediaLibrary.create({ folderTitle: lowerFolderTitle })
    res.status(201).json({
        status: "success",
...
const deleteFiles = async (req, res, next) => {
    ...
    if (!isValidObjectId(folderId) || !Array.isArray(fileIds) || !fileIds.every(isValidObjectId)) {
        const err = appError.create("Invalid folder id or file ids", 400, STATUS.FAIL)
        return next(err)
    }
    const folder = await MediaLibrary.findById(folderId)
    if (!folder) {
    ...
    const filesToDelete = folder.files.filter(file => fileIds.includes(file._id.toString()))
    if (filesToDelete.length === 0) {
        const err = appError.create("No files found to delete", 404, STATUS.FAIL)
        return next(err)
    }
    // Delete files from cloudinary
    await Promise.all(filesToDelete.map(file => cloudinary.uploader.destroy(file.publicId)))
    // Remove files from folder
    folder.files = folder.files.filter(file => !fileIds.includes(file._id.toString()))
    await folder.save()
    res.status(204).end()
}
...
const deleteFolder = async (req, res, next) => {
    ...
    if (folder.folderTitle === "profile_pictures") {
        const err = appError.create("You cannot delete this folder", 400, STATUS.FAIL)
        return next(err)
    }
