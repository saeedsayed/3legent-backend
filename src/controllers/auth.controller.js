...
export const sendOtpToEmail = async (req, res, next) => {
    ...
    await sendEmail({ to: targetUser.email, subject: "Your OTP Code", html: `<p style="font-size: 16px; text-align: center;">Your OTP code is <b style="font-size: 24px; display:block">👉 ${otp} 👈</b> It will expire in 10 minutes.</p>` })
    res.json({
        status: STATUS.SUCCESS,
...
export const forgotPassword = async (req, res, next) => {
    ...
    targetUser.otpExpiresAt = otpExpiresAt
    await targetUser.save()
    await sendEmail({ to: email, subject: "Your OTP Code", html: `<p style="font-size: 16px; text-align: center;">Your OTP code is <b style="font-size: 24px; display:block">👉 ${otp} 👈</b> It will expire in 10 minutes.</p>` })
    res.json({
        status: STATUS.SUCCESS,
        message: "OTP sent to your email successfully"
    })
