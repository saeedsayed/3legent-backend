...
const addToCart = async (req, res, next) => {
    ...
    try {
        ...
        if (!userCart) {
            ...
            const filteredProducts = userCart.products.filter(p => p.product && p.product._id.toString() !== productId);
            filteredProducts.push({ product: productId, quantity });
            userCart.products = filteredProducts;
        }
        userCart.totalPrice = getCartTotalPrice((await userCart.populate('products.product')).products);
        await userCart.save();
...
const removeFromCart = async (req, res, next) => {
    ...
    try {
        ...
        userCart.products = userCart.products.filter(p => p.product._id.toString() !== productId);
...
const clearCart = async (req, res, next) => {
    try {
        ...
        await userCart.save();
        res.json({ status: STATUS.SUCCESS, data: userCart, message: "Cart cleared" });
    } catch (error) {
        next(error);
    }
};
